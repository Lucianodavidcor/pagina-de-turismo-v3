...
import { createPost, getPublicPosts, ForumPost } from '../services/forum';
...
const ForumPage: React.FC = () => {
  ...
  // Estado visual para hover en estrellas
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans flex flex-col">
      ...
      <div className="bg-gradient-to-r from-cyan-700 to-blue-800 text-white pt-24 pb-32 px-6 text-center relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            ...
            <p className="text-lg md:text-xl text-cyan-100 font-light">
                Descubre secretos, comparte tus historias y ayuda a otros a explorar la Patagonia.
            </p>
          </div>
      </div>
      ...
      <main className="container mx-auto px-6 -mt-20 relative z-20 pb-20 flex-1 max-w-5xl">
        ...
        {/* --- FORMULARIO DE PUBLICACIÓN (ESTILO TARJETA FLOTANTE) --- */}
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden mb-16 animate-fade-in-up">
            ...
            <div className="bg-gray-50 px-8 py-6 border-b border-gray-100 flex justify-between items-center">
                <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                    <span className="bg-cyan-100 text-cyan-600 w-8 h-8 rounded-full flex items-center justify-center text-sm">
            ...
            <div className="p-8">
                ...
                {!user ? (
                    <div className="text-center py-10">
                        ...
                        <a href="/login" className="inline-block bg-cyan-600 text-white font-bold py-3 px-8 rounded-full hover:bg-cyan-700 transition shadow-lg hover:shadow-cyan-200">
                            Iniciar Sesión
                        </a>
                    </div>
                ) : (
                    <form onSubmit={handleSubmit} className="space-y-8">
                        <div className="grid md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    <select 
                                        className="w-full pl-4 pr-10 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none appearance-none bg-white transition hover:border-gray-400"
                            ...
                            {/* Selector Estrellas Interactivo */}
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-1 h-[46px]">
                                    ...
                                    <span className="ml-3 text-sm font-medium text-gray-400 bg-gray-100 px-2 py-1 rounded">
                                        {hoverRating || newPost.rating} / 5
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea 
                                className="w-full p-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none h-32 resize-none transition hover:border-gray-400" 
                                placeholder="¿Qué fue lo que más te gustó? ¿Algún consejo para otros viajeros?"
                                value={newPost.text}
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-4">
                                {/* Zona de Drop (Input) */}
                                <label className={`
                                    w-32 h-32 flex flex-col items-center justify-center border-2 border-dashed rounded-xl cursor-pointer transition-all
                                    ${uploading ? 'bg-gray-100 border-gray-300 cursor-wait' : 'border-cyan-300 bg-cyan-50 hover:bg-cyan-100 hover:border-cyan-500'}
                                `}>
                                    {uploading ? (
                                        <div className="animate-spin text-cyan-600 text-2xl"><i className="fas fa-circle-notch"></i></div>
                                    ) : (
                                        <>
                                            <i className="fas fa-camera text-2xl text-cyan-600 mb-2"></i>
                                            <span className="text-xs font-bold text-cyan-700">Agregar</span>
                                        </>
                                    )}
                                ...
                                {newPost.images.map((url, i) => (
                                    <div key={i} className="relative w-32 h-32 group animate-fade-in">
                                        <img src={url} className="w-full h-full object-cover rounded-xl shadow-sm border border-gray-200" alt="preview" />
                                        <button 
                                            type="button"
                                            onClick={() => removeImage(i)}
                                            className="absolute -top-2 -right-2 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs shadow-md opacity-0 group-hover:opacity-100 transition-opacity transform hover:scale-110"
                                        >
                        ...
                        <div className="pt-4 border-t border-gray-100">
                            <button type="submit" className="w-full sm:w-auto float-right bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold py-3 px-8 rounded-xl hover:shadow-lg hover:from-cyan-600 hover:to-blue-700 transition transform active:scale-95 flex items-center gap-2 justify-center">
        ...
        <div>
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="text-sm text-gray-500 bg-white px-3 py-1 rounded-full shadow-sm border border-gray-100">
                    Mostrando {posts.length} reseñas
                </div>
            </div>
            ...
            {loading ? (
                <div className="grid gap-6">
                    {[1, 2, 3].map(i => (
                        <div key={i} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 animate-pulse flex gap-4">
                ...
                <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-200">
                    ...
                    <p className="text-gray-400">¡Sé el primero en contar tu historia!</p>
                </div>
            ) : (
                <div className="grid gap-6">
                    {posts.map((post) => (
                        <div key={post.id} className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-300 flex flex-col md:flex-row gap-6">
                                ...
                                <div className="w-16 h-16 rounded-full p-1 bg-gradient-to-tr from-cyan-400 to-blue-500">
                                    <div className="w-full h-full rounded-full bg-white flex items-center justify-center overflow-hidden">
                                        {post.avatar_url ? (
                                            ...
                                            <span className="text-2xl font-bold text-gray-400">{post.author.charAt(0).toUpperCase()}</span>
                                        )}
                                    </div>
                                </div>
                                <div>
